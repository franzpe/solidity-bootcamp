...
const deployTask = task(
  ...
  async (taskArgs: { proposals: string[] }, hre) => {
    const { proposals } = taskArgs;
    ...
    const ballotContract = await ballotContractFactory.deploy(convertStringArrayToBytes32(hre, proposals), {});
